...
namespace CLARA::CLASM {
	...
	template<> struct Diagnostic<DiagCode::UnexpectedOperand> {
		constexpr static auto name = "unexpected operand"sv;
		...
		auto formatMessage() const
		{
			...
			if (!numExpected) {
				return fmt::format("instruction takes no operands, {} provided"sv, numGiven);
			}

			if (numGiven > 1) {
				return fmt::format("expected {} {}, found {}"sv, numExpected, numExpected == 1 ? "operand"sv : "operands"sv, numGiven);
			}
	...
	template<> struct Diagnostic<DiagCode::LabelRedefinition> {
		constexpr static auto name = "label redefinition"sv;
	...
	template<> struct Diagnostic<DiagCode::UnresolvedLabelReference> {
		constexpr static auto name = "unresolved label reference"sv;
	...
	template<> struct Diagnostic<DiagCode::InvalidKeywordArgCount> {
		constexpr static auto name = "incorrect number of keyword arguments"sv;
		...
		{
			return fmt::format(
				"expected {} {} for \'{}\' keyword, {} given",
				numExpected,
				numExpected == 1 ? "argument"sv : "arguments"sv,
				to_string(keyword),
				numGiven
			);
		}
